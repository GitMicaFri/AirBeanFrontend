...
function YourOrder() {
    ...
    const getTotalPrice = () => {
        // Vi börjar på totalPrice = 0
        let totalPrice = 0

        // För varje produktnamn i Cart
        productNamesInCart.forEach(productName => {
            // Kolla hur många av denna produkt som finns i cart
            const productAmountInCart = products[productName]

            // Hitta produkten i "productList"
            const currentProduct = productList.find(product => product.name === productName)
    ...
    return (
        <div className="cart">
            ...
            {productNamesInCart.map((productName, index) => <SingleCartItem
            ...
            <Link to="/orderonitsway" className="takeMoneyBtn" onClick={clearCart} >Take My Money!</Link>
        </div>
    )
}
...
const SingleCartItem = (props) => {
    ...
    const totalPriceForThisProduct = getTotalPriceForThisProduct(props.productName, props.productAmount)

    const addThisProduct = () => props.addProduct(props.productName)
    const removeThisProduct = () => props.removeProduct(props.productName)

    return <li className="single-cart-item">
        <div className="product-details">
            <h3>{props.productName}</h3>
            <p>{totalPriceForThisProduct} kr</p>
        </div>
        <div className="product-control">
            <button className="increase" onClick={addThisProduct} >+</button>
            <p>{props.productAmount}</p>
            <button className="decrease" onClick={removeThisProduct}>-</button>
        </div>
    </li>
}
